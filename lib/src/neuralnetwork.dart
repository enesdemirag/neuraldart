...
class NeuralNetwork {
  ...
  // Training data
  static List<Dataset> dataset = [];
  
  static void forward(List<double> inputs) {
    // Bring the inputs into the input layer
    layers.insert(0, Layer(inputs));
    
    // Forward propagation
    for(int i = 1; i < layers.length; i++) { // Starts from 1st hidden layer
  ...
  static void backward(double learning_rate, Pair data) {
    ...
    for(int i = 0; i < layers[output_layer_index].neurons.length; i++) { // For each output
      double output = layers[output_layer_index].neurons[i].value;
      double target = data.expected_output[i];
      double derivative = output - target;
      double delta = derivative * (output * (1 - output));
      layers[output_layer_index].neurons[i].gradient = delta;
      
      for(int j = 0; j < layers[output_layer_index].neurons[i].weights.length; j++) { // and for each of their weights
        double previous_output = layers[output_layer_index - 1].neurons[j].value;
        double error = delta * previous_output;
        layers[output_layer_index].neurons[i].weights_old[j] = layers[output_layer_index].neurons[i].weights[j] - learning_rate * error;
      }
    }
    ...
    for(int i = output_layer_index - 1; i > 0; i--) { // Backward
      for(int j = 0; j < layers[i].neurons.length; j++) { // For all neurons in that layers
        ...
        double gradient_sum = utils.sumGradient(j, i + 1);
